	...
	BatFallback batFallback)
{
	public async Task<StartupResult> RunAsync(string[] args, CancellationToken ct = default)
	{
		try
		{
			...
			logger.Info($"StartupMode={config.StartupMode}; FallbackMode={config.FallbackMode}; Backend={config.BackendStartCommand}; Browser={config.BrowserStartUrl}");
			...
			if (!orchestrationResult.Success)
			{
				...
				if (string.Equals(config.FallbackMode, "auto", StringComparison.OrdinalIgnoreCase)
					...
					&& !string.Equals(orchestrationResult.Stage, "browser-launch-basic", StringComparison.OrdinalIgnoreCase))
				{
				...
				{
					"ai-launch" => (int)WrapperExitCode.LlamaLaunchFailed,
					"ai-readiness" => (int)WrapperExitCode.LlamaReadinessFailed,
					"backend-launch" => (int)WrapperExitCode.WebUiLaunchFailed,
					"backend-readiness" => (int)WrapperExitCode.WebUiReadinessFailed,
					"routing-launch" => (int)WrapperExitCode.WebUiReadinessFailed,
					"routing-readiness" => (int)WrapperExitCode.WebUiReadinessFailed,
					"geo-readiness" => (int)WrapperExitCode.WebUiReadinessFailed,
