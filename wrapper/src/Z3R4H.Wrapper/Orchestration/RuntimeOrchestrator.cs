...
public sealed class RuntimeOrchestrator(WrapperLogger logger, ReadinessProbe readinessProbe, StartupScreen startupScreen)
{
	public async Task<(bool Success, WrapperErrorCategory? Category, string Stage, string Message)> StartAsync(WrapperConfig config, CancellationToken ct = default)
	{
		...
		var routingReady = await EnsureRoutingAndGeoHealthAsync(config, ct);
		var basicMode = !routingReady.RoutingReady;
		if (basicMode)
		{
			logger.Error(WrapperErrorCategory.ProbeFailure, routingReady.RoutingMessage);
		...
		if (!routingReady.GeoReady)
		{
			startupScreen.StepFailure("Geo health", routingReady.GeoMessage);
			return (false, WrapperErrorCategory.Timeout, "geo-readiness", routingReady.GeoMessage);
		}
		startupScreen.StepSuccess("Geo health ready");
	...
	private async Task<(bool RoutingReady, string RoutingMessage, bool GeoReady, string GeoMessage)> EnsureRoutingAndGeoHealthAsync(WrapperConfig config, CancellationToken ct)
	{
		var routingReady = true;
		var routingMessage = "Routing ready.";
		...
		if (!string.IsNullOrWhiteSpace(config.ValhallaStartCommand))
		{
			...
			{
				routingReady = false;
				routingMessage = valhallaStart.Message;
			}
		}
		...
		if (routingReady)
		{
			...
			{
				routingReady = false;
				routingMessage = valhallaReady.Message;
			}
		}
		...
		return (routingReady, routingMessage, geoReady.Success, geoReady.Message);
	}
